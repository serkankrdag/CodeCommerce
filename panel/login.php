<?php function login() { global $db;

    if (isset($_POST['login'])) {
        $login = $db->prepare("select COUNT(*) from managers where email = ? and password = ?");
        $login->execute([$_POST['email'],md5(sha1($_POST['password']))]);
        $count = $login->fetchColumn();
        if ($count>0) {
            $login = $db->prepare("select * from managers where email = ? and password = ?");
            $login->execute([$_POST['email'],md5(sha1($_POST['password']))]);
            $sorgu = $login->fetch();

            $_SESSION["loginp"] = 'success';
            $_SESSION["id"] = $sorgu['id'];
            $_SESSION["name"] = $sorgu['name'];
            $_SESSION["email"] = $sorgu['email'];
            $_SESSION["phone"] = $sorgu['phone'];
            $_SESSION["picture"] = $sorgu['picture'];
            $_SESSION["country"] = $sorgu['country'];
                    ...
                    <form class="form w-100" method="post">
                        <div class="text-center mb-10">
                            ...
                            <div class="text-gray-400 fw-bold fs-4">New Here?
                                <a href="../../demo7/dist/authentication/flows/basic/sign-up.html" class="link-primary fw-bolder">Create an Account</a>
                            </div>
                        </div>
                        ...
                        <div class="text-center">
                            <button name="login" type="submit" id="kt_sign_in_submit" class="btn btn-lg btn-primary w-100 mb-5">
