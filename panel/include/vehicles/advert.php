<?php function advert() { global $db;
        ...
        if ($_FILES['picture']) {
            ...
            if ($_FILES["picture"]["size"]  > 1000000) {
                ...
                if ($dosyaUzantisi != "jpeg" && $dosyaUzantisi != "jpg" && $dosyaUzantisi != "png") {
                    ...
                    $sorgu = $db->query("select * from advert where id='1'")->fetch();
                    $file = "../upload/advert/".$sorgu['picture'];
                    if ($sorgu['picture'] != '') {
                        unlink($file);
                    }
                }
        ...
        if($ekle) {
            $controle=
                ...
                <div class="alert alert-success d-flex align-items-success p-5">
                    ...
                    <div class="d-flex flex-column">
                        <h4 class="mb-1 text-dark">Reklam Afişi Güncellendi</h4>
                        <span>Reklam Afişi Güncelleme işlemi başarılı bir şekilde gerçekleşti.</span>
                    </div>
                </div>
                ';
        } elseif (!$ekle) {
            $controle=
                ...
                <div class="alert alert-danger d-flex align-items-danger p-5">
                    ...
                    <div class="d-flex flex-column">
                        <h4 class="mb-1 text-dark">Reklam Afişi Güncellenemedi</h4>
                        <span>Reklam Afişi Güncelleme işlemi başarısız lütfen bütün alanları doldurun.</span>
                    </div>
                </div>
                ';
        }
    ...
    $advert = $db->query("select * from advert where id = '1' limit 1")->fetch();

    breadcrumb('Kontrol Panel','Araçlar','Reklam Güncelleme'); ?>
                                ...
                                <div class="card-body text-center pt-0">
                                    <style>
                                        .image-input-placeholder1 {
                                            background-image: url(../upload/advert/<?=$advert['picture']?>);
                                        }
                                    </style>
                                    ...
                                    <div class="text-muted fs-7">Reklam küçük resmini ayarlayın. Yalnızca *.png, *.jpg ve *.jpeg resim dosyaları kabul edilir</div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card card-flush py-4">
                                ...
                                <div class="card-body pt-0">
                                    <div class="mb-10 fv-row fv-plugins-icon-container">
                                        <label class="required form-label">Reklam Başlığı</label>

                                        <input type="text" name="title" class="form-control mb-2" placeholder="Reklam başlığı" value="<?=$advert['title']?>">

                                        <div class="text-muted fs-7">Bir reklam başlığı gereklidir ve benzersiz olması önerilir.</div>
                                        <div class="fv-plugins-message-container invalid-feedback"></div></div>

                                    <div>
                                        <label class="form-label">Tanım</label>

                                        <textarea name="description" class="form-control mb-2" id="description" cols="30" rows="10"><?=$advert['description']?></textarea>
                                        <div class="text-muted fs-7">Daha iyi görünürlük için reklama bir açıklama ayarlayın.</div>
                                    </div>
                                </div>
                            </div>
