<?php function brandsEdit() { global $db; global $menu;
        ...
        if ($_FILES) {
            ...
            if ($_FILES["picture"]["size"]  > 1000000) {
                ...
                if ($dosyaUzantisi != "jpeg" && $dosyaUzantisi != "jpg" && $dosyaUzantisi != "png") {
                    ...
                    $sorgu = $db->query("select * from brands where id='$menu[4]'")->fetch();
                    $file = "../upload/brands/".$sorgu['picture'];
                    if (isset($sorgu['picture']) && !empty($sorgu['picture'])) {unlink($file);}
                }
            }
        }
        ...
        if($ekle) {
            $controle=
                ...
                <div class="alert alert-success d-flex align-items-success p-5">
                    ...
                    <div class="d-flex flex-column">
                        <h4 class="mb-1 text-dark">Marka Eklendi</h4>
                        <span>Marka ekleme işlemi başarılı bir şekilde gerçekleşti.</span>
                    </div>
                </div>
                ';
        } elseif (!$ekle) {
            $controle=
                ...
                <div class="alert alert-danger d-flex align-items-danger p-5">
                    ...
                    <div class="d-flex flex-column">
                        <h4 class="mb-1 text-dark">Marka Eklenemedi</h4>
                        <span>Marka ekleme işlemi başarısız lütfen bütün alanları doldurun.</span>
                    </div>
                </div>
                ';
        }
    ...
    $brand = $db->query("select * from brands where id = $menu[4] limit 1")->fetch();

    breadcrumb('Kontrol Panel','Markalar','Marka Düzenleme'); ?>
                            ...
                            <div class="card card-flush py-4">
                                ...
                                <div class="card-body text-center pt-0">
                                    <style>
                                        ...
                                        <?php if (isset($brand['picture']) && !empty($brand['picture'])) { ?>
                                            background-image: url(../upload/brands/<?=$brand["picture"]?>);
                                        <?php } else { ?>
                                    ...
                                    <div class="text-muted fs-7">Marka küçük resmini ayarlayın. Yalnızca *.png, *.jpg ve *.jpeg resim dosyaları kabul edilir</div>
                                </div>
                            </div>
                            <div class="card card-flush py-4">
                                <div class="card-header">
                                ...
                                <div class="card-body pt-0">
                                    <select name="status" class="form-select mb-2" data-control="select2" data-placeholder="Bir seçenek seçin">
                                        <option></option>
                                        <option <?= $selected = ($brand['status'] == '1') ? 'selected' : ''; ?> value="1">Aktif</option>
                                        <option <?= $selected = ($brand['status'] == '0') ? 'selected' : ''; ?> value="0">Pasif</option>
                                    </select>
                                    <div class="text-muted fs-7">Marka durumunu ayarlayın.</div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card card-flush py-4">
                                ...
                                <div class="card-body pt-0">
                                    <div class="mb-10 fv-row fv-plugins-icon-container">
                                        ...
                                        <input type="text" name="name" class="form-control mb-2" placeholder="Marka adı" value="<?=$brand['name']?>">

                                        <div class="text-muted fs-7">Bir marka adı gereklidir ve benzersiz olması önerilir.</div>
                                        <div class="fv-plugins-message-container invalid-feedback"></div></div>

                                    <div>
                                        <label class="form-label">Tanım</label>

                                        <textarea class="form-control mb-2" name="description" id="description" cols="30" rows="10"><?=$brand['description']?></textarea>
                            ...
                            <div class="card card-flush py-4">
                                ...
                                <div class="card-body pt-0">
                                    <div class="mb-10">
                                        ...
                                        <input type="text" class="form-control mb-2" name="metaTitle" placeholder="Meta etiket başlığı" value="<?=$brand['metaTitle']?>">
                                    ...
                                    <div class="mb-10">
                                        ...
                                        <textarea class="form-control mb-2" name="metaDescription" id="metaDescription" cols="30" rows="5"><?=$brand['metaDescription']?></textarea>
                                        <div class="text-muted fs-7">Artan SEO sıralaması için markaya bir meta etiket açıklaması ayarlayın.</div>
                                    </div>

                                    <div>
                                        <label class="form-label">Meta Tag Keywords</label>

                                        <input value="<?=$brand['metaTitle']?>" name="metaKeywords" class="form-control mb-2" tabindex="-1">
